...
/*
Add a button "Insert icons" to post editor that triggers a modal window ( Thickbox : http://codex.wordpress.org/ThickBox )
*/

function smart_ifw_select_button() {
	global $pagenow, $wp_version;
	global $smart_ifw;
    $smart_ifw = get_option('_smart_ifw_options');
	$output = '';

	/** Output generated after page conditionals */
	if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) )  ) {
		/* Media button for older wordpress versions */
		if ( version_compare( $wp_version, '3.5', '<' ) ) {
			$img = '<img src="' . SMART_IFW_URL . 'images/menu-icon.png" alt="' . __( 'Add Icons', 'smart_ifw' ) . '"/>';
			$output = '<a href="#TB_inline?width=640&inlineId=icon-select" class="thickbox smart-ifw-thickbox" title="' . __( 'Add Icons', 'smart_ifw' ) . '">' . $img . '</a>';
		} else {
			$img = '<span class="wp-media-buttons-icon dashicons dashicons-plus" id="add-icon-button"></span>';
			$output = '<a href="#TB_inline?width=640&inlineId=icon-select" class="thickbox button smart-ifw-thickbox" title="' . __( 'Add Icons', 'smart_ifw' )  . '" style="padding-left: .4em;">' . $img . __( 'Add Icons', 'smart_ifw' ) . '</a>';
		}
	}
	echo $output;
}
add_action( 'media_buttons', 'smart_ifw_select_button', 10);


function smart_ifw_select_thickbox() {
	...
	if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) { ?>
		...
		<script type="text/javascript">
            ...
            function addIcon() {
			        var icon = jQuery('#select-ifw-icon').val();
					

					//if ( $('input[name="switch_rad"]').is(':checked') ) {
					
					// } 
			var radius = (jQuery('input[name="switch_rad"]').prop("checked")) ? ' radius' :'';
			var shadow = (jQuery('input[name="switch_shad"]').prop("checked")) ? ' shadow' :'';

                    
                if ('' === icon) {
                    alert('<?php _e( "No icon selected", "smart_ifw" ); ?>');
                    return;
                }
				
               // Send the shortcode to the editor
                window.send_to_editor('[<?php echo $smart_ifw['icon_code']; ?> name="fa ' + icon + '"' + radius + ''+ shadow +']'); 
            }
           jQuery(document).ready(function ($) {
				$(".chosen-select").chosen({width: '100%'});
                $('#select-ifw-icon').change(function () {
				...
				$("#select-ifw-icon option").each(function(){
				if ($(this).val() != '') {
				var eicon = $(this).val();
				var eicon = '<span class="fa '+ eicon +'"></span> ';
		...
		<div id="icon-select" style="display: none;">
			<div class="smart-ifw wrap" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">
				<p><?php echo __( '<strong>Find, Select, & Add Icons</strong>', 'smart_ifw' ) ?></p>
				<?php if( is_admin() ) : ?>
					...
					<table class="form-table">
						...
						<td></td>
						<td>	<div>
						<?php if ($smart_ifw['switch_fa'] == 'on') { ?>
						<select id="select-ifw-icon" class="chosen-select" style="clear: both; display: block; margin-bottom: 1em; margin-top: 1em;">
						<option value=""><?php _e( 'Select Icon', 'smart_ifw' ); ?></option>
						<?php
							$icons = smart_ifw_fa_icnlist();
							foreach ( $icons as $icon => $unicode ) {
								echo '<option value="' . $icon . '" class="fa fa-'.$icon.'" style="clear: both; display: block;">' . $unicode . '; '.$icon.'</option>';
							}
						?>
					</select>
					<?php } else { echo '<span style="font-style:italic;font-weight:900;">Icons are turned off</span>'; }?>
				</div></td>
						</tr>
						<tr>
						<td>
									<div class="left"><strong>Radius</strong></div>
						</td>
						<td>
									 <div class="onoffswitch pad-left left">
									 <input type="checkbox" name="switch_rad" class="onoffswitch-checkbox" id="switch_rad" <?php    if (isset($checked)) {  echo $checked; }?>>
									 <label class="onoffswitch-label" for="switch_rad">
									 ...
									 <span class="onoffswitch-switch"></span>
												...
												<tr>
						<td>
									 <div class="left"><strong>Shadow</strong></div>
						</td>
						<td>
									 <div class="onoffswitch pad-left left">
									 <input type="checkbox" name="switch_shad" class="onoffswitch-checkbox" id="switch_shad" <?php    if (isset($checked)) {  echo $checked; }?>>
									 <label class="onoffswitch-label" for="switch_shad">
									 <span class="onoffswitch-inner"></span>
									 <span class="onoffswitch-switch"></span>
									 </label>
									 </div>
						...
						<tr>
						<td>
									 <div class="left"><strong>Animation</strong></div>
						</td>
						<td>
						...
						<tr>
						<td>
									 <div class="left"><strong>Colour</strong></div>
						</td>
						<td>
									 <div class="onoffswitch proswitch pad-left left">
									 <input type="checkbox" name="switch_pro" class="onoffswitch-checkbox" id="switch_pro">
						...
						<tr>
						<td>
									 <div class="left"><strong>Size</strong></div>
						</td>
						<td>
									 <div class="onoffswitch proswitch pad-left left">
									 <input type="checkbox" name="switch_pro" class="onoffswitch-checkbox" id="switch_pro">
				...
				$colors = smart_ifw_buttons();
				if( $colors ) { ?>
				<div id="smart-ifw-button-select" style="display: none;">
					<select id="smart-ifw-icon-button" style="clear: both; display: block; margin-bottom: 1em;">
						<option value=""><?php _e('Insert button with icon', 'smart_ifw'); ?></option>
						<?php
				...
				<?php } ?>
				<p class="submit">
					<input type="button" id="smart-ifw-add-icons" class="button-primary" value="<?php echo  __( 'Add Icon', 'smart_ifw' ) ; ?>" onclick="addIcon();" />
					<a id="smart-ifw-dont-add-icons" class="button-secondary" onclick="tb_remove();" title="<?php _e( 'Cancel', 'smart_ifw' ); ?>"><?php _e( 'Cancel', 'smart_ifw' ); ?></a>
				</p>
...
add_action( 'admin_footer', 'smart_ifw_select_thickbox' );

function smart_ifw_is_pro () {


}

function smart_ifw_buttons () {

}

function smart_ifw_fa_icnlist(){
